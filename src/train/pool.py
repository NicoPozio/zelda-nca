...
class SamplePool:
    ...
    def __init__(self, rooms, pool_size: int, hidden_channels: int,
        ...
        Args:
            ...
            with_aux: serve per modello con distanza BFS
        """
        ...
        self.target = self.rooms[idx].to(device)                      
        self.state = to_nca_state(self.target, hidden_channels).to(device) 
        self.aux = self._aux_for(idx)                                 
    ...
    def reseed(self, slots):
        """Sostituisce gli slot indicati con stanze non danneggiate
